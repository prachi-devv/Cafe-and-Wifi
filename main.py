...
app.config['SECRET_KEY'] = 'f02c440a136e70454890c0c8d36fbb6197e4d7e321486b38f1d6162070f7be04'
...
def home():
    ...
    if filter_conditions and not Cafes.query.filter(and_(*filter_conditions)).all():
        return render_template("no_results.html", selected_filters=selected_filters)
    
    # Render index.html with filtered or all cafes
    return render_template("index.html", all_cafes=Cafes.query.filter(and_(*filter_conditions)).all() or cafes, selected_filters=selected_filters, cafes_count=cafes_count)
